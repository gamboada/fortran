...
constrants = open('constants.inc.txt')
"""
...
n = 0  #number of groups/matrices in a single file 
b = 0  # check for dash lines to create empty list of lists
m = 1  # keeps track of version of DIPOLE_MID
ran_gauss = [] 
"This finds how many matrixes are in the file"
with open(data, "r") as f:
    for i, dash in enumerate(f): #go through line by line whole doc
        dash = dash.strip() #clean empty space before and after each line
        if "-------------" in dash: #count section using lines by incrementing b 
            b += 1
Matrix = []  # empty ubberMatrix

for i in range (0,b): #initialize individual matrices withing ubbermatrix using b
    M_i = []
for i in range (0,b):
    Matrix.append(M_i)
...
#labels from huge .txt matrix files
names = []
regions = []
offsets = []
lengths = []
...
with open(data, "r") as d:
    for i, line in enumerate(d): #dash and line simply designate the enumerate func
        line = line.strip()
        if "-------------" in line:
            n += 1  
        elif line.startswith('!N'): #does it have the name 
            x = line[7:]  # cut out the label in .txt
            if 'DIPOLE_MID' in x: #keep track of multiple dipole mids
                new_x = x + '_{0}'.format(m) #increment by m 
                names.append(new_x) 
                m += 1
            else:
                names.append(x)
        elif line.startswith('!R'): #find desired line and append label to building list
            ...
            Matrix[n].append(list(line.split())) # append individual matrices corresponding to an n to ubbermatrix 
print("")
...
for i in range(0,b):
    Matrix[i] = [list(map(float, sublist)) for sublist in Matrix[i]] # look at a sublist (line) of strings within M_i, map them to floats  
for i in range(0,b):
    Matrix[i] = np.array(Matrix[i]) #NUMPYYY arrays, string from list loses its commmas
...
""" #copy from fortran initializing masses 
m2 = me2
if p_flag == 0:
    m2 = me2
elif p_flag == 1:
...
spectrometer = config['DEFAULT']['spectrometer (1']
...
beam_x_offset = config['DEFAULT']['beam x offset (cm)  (+x']
beam_y_offset = config['DEFAULT']['beam y offset (cm)  (+y']
target_z_offset = config['DEFAULT']['target z offset (cm) (+z']
spectrometer_x_offset = config['DEFAULT']['spectrometer x offset (cm) (+x']
...
particle_id = config['DEFAULT']['particle identification']
...
print("")
print("monte Carlo interations = ", monte_carlo_interations)
"""
...
for i in range(1000):
    value = gauss(0, 2)
    ran_gauss.append(value)
print("")
print("array of random Guassian values with mean of 0 and standard deviation of 2")
print(ran_gauss)
